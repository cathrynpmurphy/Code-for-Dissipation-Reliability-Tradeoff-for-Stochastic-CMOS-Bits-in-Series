...
#######################################
#
# L and vDD_vT are read from the command line (`julia SteadyStateDMRG.jl <L> <vDD_vT>`)
# so the 16 production points (L=1,3,5,7 x Vdd/VT=1.1,1.2,1.3,1.4) can be driven from a
# shell loop without hand-editing this file each time -- e.g.:
#
#   for L in 1 3 5 7; do for V in 1.1 1.2 1.3 1.4; do
#     julia SteadyStateDMRG.jl $L $V
#   done; done
#
# Running with no arguments keeps the previous single-run behavior (defaults below match
# what used to be hard-coded here).
...
L = length(ARGS) >= 1 ? parse(Int, ARGS[1]) : 7 #The number of units in our chain
...
##########################################################################
## WARM-START DMRG (Sec. IV D of the paper) ##############################
##########################################################################
# DMRG is not run once at full resolution. Instead we begin from the
# uniform MPS above and converge it in stages, using a small maximal bond
# dimension (chi = MD) and few retained single-unit basis vectors
# (d = nbasis) at first. As the MPS converges over a stage's cycles, both
# of these cutoffs are increased together, ending at the production
# values (d = 32, chi <= 100) used to report results in Sec. V. Increasing
# resolution gradually like this, rather than starting DMRG directly at
# the production settings, is what makes the largest cases (e.g. L = 7,
# Vdd/VT = 1.4, which needed ~5400 total single-site sweeps in the paper)
# converge reliably.
#
# Each row of the schedule is one stage: [nbasis (d), maxdim (chi), ncycles].
# We step d up by 2 basis vectors per stage, from the starting point
# (d = 12) to the production endpoint (d = 32), giving 11 stages. Each
# stage runs ~540 single-site sweeps (matching the paper's total of
# ~5400 sweeps for the hardest case, L = 7, Vdd/VT = 1.4, spread evenly
# over the 11 stages); with StepNum DMRG iterations per cycle below,
# that's ncyclesPerStage = 540 / StepNum cycles per stage. The bond
# dimension chi is ramped in step with d, from a small starting value up
# to the production cap (chi <= 100), per "both cutoffs are increased
# together" in Sec. IV D. Ramped 80 -> 100 
...
# Created up front, not just before the final Pss.h5 save below -- the per-cycle
# Energies_Pss_*.jld2 checkpoint inside the loop below writes here starting on the very
# first cycle.
mkpath("Data")
...
for stage = 1:size(warmStartSchedule,1)
	...
	# Transform the MPO, and the best current guess for Psi, into this stage's basis:
	...
	for t=1:ncycles
		...
		#Save the outputs:
		push!(energies,Henergy) # This should converge to zero
...
#already transformed back into the occupation basis, i.e. Pss(v0,v1,...vL)
Pss = Psi
# Save the steady state MPS as a .h5 file, named for the (L, vDD_vT) point it was
# computed at, so ExcitedStateDMRG.jl and ComputeDissipation.jl can each find the right
# one when run for the same point, and so all 16 production points' outputs coexist in
# this one Data/ folder without overwriting each other.
h5open("Data/Pss_Vdd$(vDD_vT)_L$(L).h5", "w") do f
    write(f, "Pss", Pss)
end
